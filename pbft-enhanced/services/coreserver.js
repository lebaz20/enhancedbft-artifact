...
class Coreserver {
  ...
  // Classify shards as healthy or broken and return the info needed for merging.
  //
  // Returns:
  //   healthyShards – shard indices that already have consensus (left untouched)
  //   brokenShards  – { shard, healthy } entries for shards that have lost consensus
  //   nodesPerShard – canonical shard size derived from shardNodeCounts
  buildShardMergeMap() {
    ...
    return { healthyShards, brokenShards, nodesPerShard }
  }

  // Execute shard merging for all broken shards.
  //
  // Design:
  //   - Healthy shards are left completely alone.
  //   - Honest nodes from ALL broken shards are pooled together.
  //   - The pool is split into groups of NUMBER_OF_NODES_PER_SHARD (NPS).
  //   - A remainder group is kept only if it has >= MIN_VIABLE_REMAINDER nodes.
  //   - Each group becomes a MERGED_N shard with its own minApprovals computed
  //     from the group's actual size (2f+1 where f = floor((size-1)/3)).
  //   - Faulty nodes from broken shards are redirected to the first merged shard.
  applyShardMerge() {
    // Wait until all nodes have connected before computing the merge plan.
    // Two conditions release the gate (whichever comes first):
    //   1. All expected nodes connected (ideal case).
    //   2. 30 seconds elapsed since the first connection (handles crashed pods).
    ...
    const { brokenShards, nodesPerShard } = this.buildShardMergeMap()
    if (brokenShards.length === 0) return

    // Collect honest nodes from all broken shards; faulty nodes are fully isolated.
    const allHonest = [] // { port, entry }
    const brokenShardNames = new Set(brokenShards.map((b) => b.shard))

    for (const { shard } of brokenShards) {
      if (!this.sockets[shard]) continue
      for (const [port, entry] of Object.entries(this.sockets[shard])) {
        if (!entry.isFaulty) {
          allHonest.push({ port, entry })
        }
      }
    }

    // Split honest pool into NPS-sized groups.
    // Remainder >= 4: own merged shard. Remainder < 4: absorbed into last group.
    const MIN_VIABLE_REMAINDER = 4
    const nodeGroups = []
    const fullGroups = Math.floor(allHonest.length / nodesPerShard)
    for (let i = 0; i < fullGroups; i++) {
      nodeGroups.push(allHonest.slice(i * nodesPerShard, (i + 1) * nodesPerShard))
    }
    const remainder = allHonest.length - fullGroups * nodesPerShard
    if (remainder > 0) {
      if (remainder >= MIN_VIABLE_REMAINDER) {
        nodeGroups.push(allHonest.slice(fullGroups * nodesPerShard))
      } else if (nodeGroups.length > 0) {
        // Too few to form a new shard — absorb into the last group.
        nodeGroups[nodeGroups.length - 1].push(...allHonest.slice(fullGroups * nodesPerShard))
      }
    }

    logger.log(
      `SHARD MERGE MAP: brokenShards=[${[...brokenShardNames].join(', ')}]` +
        ` totalHonest=${allHonest.length} nodesPerShard=${nodesPerShard}` +
        ` → ${nodeGroups.length} merged shard(s)` +
        (remainder > 0 && remainder < MIN_VIABLE_REMAINDER && nodeGroups.length > 0
          ? ` (${remainder} remainder absorbed into last group)`
          : '')
    )

    if (nodeGroups.length === 0) return

    // --- Pass 1: build merged shard socket maps ---
    const mergedEntries = []

    for (const groupNodes of nodeGroups) {
      ...
      for (const { port, entry } of groupNodes) {
      ...
      // Compute minApprovals for this group's actual size (all nodes are honest).
      const groupSize = groupNodes.length
      const groupMinApprovals = 2 * Math.floor((groupSize - 1) / 3) + 1

      this.sockets[mergedShardIndex] = mergedSockets
      this.socketsMap[mergedShardIndex] = mergedPorts

      logger.log(
        `  MERGED_${this._mergeCounter}: ${groupSize} honest nodes` +
          ` nodesSubset=[${mergedNodesSubset.join(',')}] minApprovals=${groupMinApprovals}`
      )

      mergedEntries.push({
        mergedShardIndex,
        healthyNodes: groupNodes,
        mergedSockets,
        mergedNodesSubset,
        allWsUrls,
        minApprovals: groupMinApprovals
      })
    }

    // Remove all broken shard entries and register aliases for rate_to_core routing.
    for (const shard of brokenShardNames) {
      delete this.sockets[shard]
      delete this.socketsMap[shard]
      delete this.rates[shard]
      this._shardAliases.set(shard, mergedEntries[0].mergedShardIndex)
    }
    ...
    // --- Pass 2: send MERGE_SHARD to healthy nodes ---
    for (let i = 0; i < mergedEntries.length; i++) {
      const { mergedShardIndex, healthyNodes, mergedNodesSubset, allWsUrls, minApprovals } =
        mergedEntries[i]
        ...
        switch (data.type) {
          ...
          case MESSAGE_TYPE.rate_to_core:
            ...
            if (
              ...
              if (ENABLE_SHARD_MERGE && !this._mergeApplied) {
